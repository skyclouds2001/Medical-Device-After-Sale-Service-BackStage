  ...
  return (
    <>
      <Form form={form} name="work_order" layout="inline" className="my-5" initialValues={initialValues} onValuesChange={(_, params) => props.onFilter(params)}>
        ...
        <Form.Item name="product" label="产品" className="my-2">
          <Select placeholder="请选择产品" allowClear options={(products?.data ?? []).map(v => ({ label: v.model_name, value: v.model_id }))} notFoundContent={isLoading ? <Spin /> : '暂无数据'} className="w-[200px]" />
        </Form.Item>
