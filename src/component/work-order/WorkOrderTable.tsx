  ...
  return (
    <>
      <Table dataSource={props.workOrders} bordered rowKey="order_id" loading={props.loading} pagination={{ hideOnSinglePage: true }} expandable={{ expandedRowRender: record => <p>{record.order_description ?? '暂无更多描述'}</p> }}>
        ...
        <Table.Column width="200px" align="center" title="预约时间" dataIndex="appointment_time" key="appointment_time" render={(_, record: WorkOrder) => <>{record.appointment_time.split(' ')[0]}</>} />
        ...
        <Table.Column width="150px" align="center" title="类型" dataIndex="order_type" key="order_type" render={(_, record: WorkOrder) => <>{services.find(v => v.id === record.order_type)?.text}</>} />
